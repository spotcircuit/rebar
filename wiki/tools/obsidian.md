...
When you run `/wiki-ingest` or `/wiki-file`, the framework creates pages in `wiki/` with proper `[[page-name]]` links and `#tags`. Obsidian picks them up instantly (or on the next sync cycle in WSL).
